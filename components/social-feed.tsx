...
export default function SocialFeed() {
  ...
  return <div className="interactive-feed" tabIndex={0} aria-label="Interactive social feed. Use left and right arrow keys to change carousel slides."
    ...
    {current ? <div className="feed-layout"><div className="feed-post-list" role="group" aria-label="Choose a post">{visible.map(story => <button key={story.id} onClick={() => pick(story.id)} aria-pressed={current.id === story.id}><span>0{story.id + 1} / CAROUSEL</span><strong>{story.title}</strong><span>Explore <UiIcon name="arrow" /></span></button>)}</div>
      <article className="demo-social feed-active-post"><header><strong>orra</strong><span>Concept campaign</span></header><div className="demo-post" aria-live="polite">{current.slides[slide] === "photo" ? <Image src="/images/orra.webp" alt="ORRA citrus and cobalt skincare concept campaign" fill sizes="(max-width:700px) 85vw, 400px" /> : <div className={`demo-post-type post-${(current.id + slide) % 2 + 1}`}><span>ORRA / DAILY NOTES</span><strong>{current.slides[slide]}</strong><span>YOUR DAILY BRIGHT SIDE <UiIcon name="arrow" /></span></div>}<span className="demo-post-count">{slide + 1} / 3</span></div>
    ...
    </div> : <div className="feed-empty" role="status"><Bookmark size={32}/><h4>Your moodboard starts here.</h4><p>Save a post to collect it in this view.</p><button onClick={() => setOnlySaved(false)}>Explore the feed <UiIcon name="arrow" /></button></div>}
