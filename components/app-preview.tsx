...
export default function AppPreview() {
  ...
  return <div className="shop-demo demo-app">
    ...
    {view === "shop" && <><span className="eyebrow">THE EVERYDAY COLLECTION</span><h3>A brighter<br />kind of daily.</h3><div className="shop-demo-image"><Image src="/images/orra.webp" alt="ORRA skincare concept collection" fill sizes="(max-width:700px) 80vw, 350px" /></div><div className="shop-demo-products">{products.map((product,index) => <button key={product.name} onClick={() => { setSelected(index); setView("detail"); setMessage(""); }}><span>{product.type}</span><strong>{product.name}</strong><span>Explore ↗</span></button>)}</div></>}
    {view === "detail" && <><button className="shop-demo-back" onClick={() => setView("shop")}>← Collection</button><div className="shop-demo-image"><Image src="/images/orra.webp" alt="ORRA concept packaging" fill sizes="(max-width:700px) 80vw, 350px" /></div><span className="eyebrow">{products[selected].type}</span><h3>{products[selected].name}</h3><p>{products[selected].detail}</p><button className="shop-demo-primary" onClick={() => { setBag(items => [...items, selected]); setMessage("Added to your demo bag."); }}>Add to demo bag +</button></>}
    {view === "bag" && <><button className="shop-demo-back" onClick={() => { setView("shop"); setMessage(""); }}>← Keep exploring</button><h3>Your daily<br />bright side.</h3>{bag.length ? <><ul className="shop-demo-bag">{products.map((product,index) => { const quantity=bag.filter(item => item === index).length; return quantity ? <li key={product.name}><strong>{product.name}</strong><span>Quantity: {quantity}</span><button aria-label={`Remove one ${product.name}`} onClick={() => { setBag(items => { const next=[...items]; next.splice(next.indexOf(index),1); return next; }); setMessage(""); }}>Remove −</button></li> : null; })}</ul><button className="shop-demo-primary" onClick={() => setMessage("Demo complete. No order was placed and no payment is required.")}>Preview checkout ↗</button></> : <div className="shop-demo-empty"><p>Your bag is waiting for a little sunshine.</p><button className="shop-demo-primary" onClick={() => setView("shop")}>Explore the collection ↗</button></div>}</>}
