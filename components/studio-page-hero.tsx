...
export default function StudioPageHero({ number, label, first, second, description, target, action }: {
  ...
  return <section className="studio-page-hero section">
    ...
    <div className="studio-page-bottom"><a href={target} className="studio-page-jump" aria-label={action}><UiIcon name="down" /></a><p>{description}</p><Link href={target === "#project-brief" ? target : "/contact"} className="button">{target === "#project-brief" ? "Start your brief" : "Start something big"} <span><UiIcon name="arrow" /></span></Link></div>
  </section>;
}
