...
export default function BrandPlayground() {
  ...
  return <section className="section brand-playground" id="brand-playground" style={theme}>
    ...
    <div className="playground-modes" role="group" aria-label="Choose a brand experience">{modes.map((item, index) => <button key={item} type="button" aria-pressed={mode === item} aria-controls="brand-experience" onClick={() => setMode(item)}><span>0{index + 1}</span>{item}<span aria-hidden="true">↗</span></button>)}</div>
    <div className="brand-customizer">
      ...
      <button className="customizer-reset" onClick={() => { setPalette(0); setTypeface(0); }}>Reset ↺</button>
    ...
    <div className="playground-stage" id="brand-experience">
      ...
      <div className={`playground-device mode-${mode.toLowerCase()}`} data-app-mode={appMode}>
        ...
        {mode === "Website" && <div className="demo-web-shell"><div className="demo-device-controls" role="group" aria-label="Website preview size">{["Desktop", "Tablet", "Mobile"].map(item => <button key={item} aria-pressed={device === item} onClick={() => setDevice(item)}>{item}</button>)}</div><p className="demo-device-description" aria-live="polite">{device === "Desktop" ? "An editorial layout with room to explore." : device === "Tablet" ? "A tighter composition, with the same clear hierarchy." : "A focused, single-column experience."}</p><div className={`demo-website device-${device.toLowerCase()} ${device === "Mobile" ? "is-compact" : ""}`}><header><strong>ORRA</strong><span>Everyday, brighter.</span></header><div className="demo-web-content"><div><span className="eyebrow">A LITTLE DAILY JOY</span><h3>Your daily<br />bright side.</h3><button onClick={() => setExpanded(!expanded)} aria-expanded={expanded} aria-controls="orra-collection">{expanded ? "Close the story −" : "Explore the collection ↗"}</button></div><div className="demo-web-image"><Image src="/images/orra.webp" alt="ORRA concept collection" fill sizes="(max-width:700px) 80vw, 400px" /></div></div><div className="demo-collection" id="orra-collection" hidden={!expanded}>Simple routines. Confident colour. A skincare concept built around the small moments that brighten your day.</div></div></div>}
        {mode === "App" && <div className="demo-app"><header><strong>ORRA</strong><span>Your daily ritual</span></header><h3>A little care.<br />A brighter day.</h3><div className="demo-routine-tabs" role="group" aria-label="Choose routine">{["Morning", "Evening"].map(item => <button key={item} aria-pressed={routine === item} onClick={() => setRoutine(item)}>{item}</button>)}</div><p className="demo-routine-note">{routine === "Morning" ? "Make a little room before the day begins." : "Slow down. Make this moment yours."}</p><div className="demo-routine-steps">{["Cleanse", "Hydrate", "Take a moment"].map((step, index) => { const id = `${routine}-${step}`; const checked = steps.includes(id); return <button key={id} aria-pressed={checked} onClick={() => setSteps(checked ? steps.filter(item => item !== id) : [...steps, id])}><span>0{index + 1}</span>{step}<span>{checked ? "✓" : "+"}</span></button>; })}</div><p className="demo-routine-status" role="status">{steps.filter(item => item.startsWith(routine)).length} of 3 moments complete</p></div>}
      </div>
    </div>
